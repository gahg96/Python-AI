...
- **⚔️ AI演武场**: 多模型/多策略对比评测平台，支持规则引擎、评分系统、多轮模拟、多智能体博弈、LLM集成
...
10. **⚔️ AI演武场**: 多模型/多策略对比评测
    - 规则引擎：支持条件-动作-惩罚配置，7条默认规则模板
    - 评分系统：六维度评分（利润、风险、稳定性、合规、效率、可解释性）
    - 单场演武场：多个参赛者在同一批客户上对比
    - 多轮演武场：支持回合数、场景序列、黑天鹅事件
    - 多智能体博弈：竞争模式，多个智能体竞争同一批客户
    - LLM集成：通过model_id调用真实LLM进行决策
    - 可视化：雷达图、柱状图、回撤曲线
    - 回放系统：逐客户回放，查看详细决策过程
    - AI解释：智能分析胜者、性能对比、关键洞察
    - 导出报告：支持JSON/CSV/TXT/HTML格式
